...
class Show(Resource):
    ...
    def find_by_name(cls, title):
        ...
        if row:
            return {"show": {"type": row[1], "title": row[2], "country": row[3], "release_year": row[4], "rating": row[5], "duration": row[6], "listed_in": row[7]}}
    ...
    def post(self, title):
        if Show.find_by_name(title):
            return {"messege": f"An Show with title {title} already exists"}, 400 # 400 means bad request

        # data = request.get_json()
        data = Show.parser.parse_args()
    ...
    def put(self, title):
        ...
        if item is None:
            try:
                Show.insert(update_item)
            except:
                return {"messege": "An error occurred inserting the item."}
        else:
            try:
                Show.update(update_item)
            except:
...
class ShowsList(Resource):
    def get(self):
        ...
        query = "SELECT * FROM shows_and_movies WHERE type='TV Show' ORDER BY release_year LIMIT 20"
        result = cursor.execute(query)
        items = []
        for row in result:
            items.append({"type": row[1], "title": row[2], "country": row[3], "release_year": row[4], "rating": row[5], "duration": row[6], "listed_in": row[7]})
...
class ShowsInfo(Resource):
    ...
    def get(self, info):
        ...
        if info == "count":
            query_count = "SELECT COUNT(type) FROM shows_and_movies WHERE type='TV Show'"
            result = cursor.execute(query_count)
            result = result.fetchall()
            connection.close()
            return {"number of shows": result[0][0]}, 200
        elif info == "newest":
            query_max = "SELECT MAX(release_year) FROM shows_and_movies WHERE type='TV Show'"
            result = cursor.execute(query_max)
            result = result.fetchall()
            connection.close()
            return {"newest show year is": result[0][0]}, 200
        elif info == "oldest":
            query_min = "SELECT MIN(release_year) FROM shows_and_movies WHERE type='TV Show'"
            result = cursor.execute(query_min)
            result = result.fetchall()
            connection.close()
            return {"oldest show year is": result[0][0]}, 200
        else:
